...
- [.claude/rules/frontend-practices.md](.claude/rules/frontend-practices.md) — reuse-first, thin pages, **enforced modulith** (feature barrels `index.ts`/`server.ts`, boundaries lint — violations are CI errors), forms (RHF+Zod), service/mock parity, adapter+mapper layer, state split.
