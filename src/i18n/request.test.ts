...
describe("getRequestConfig (I18N-01 — cookie-based locale detection)", () => {
  ...
  it("test 6: messages object has nested top-level keys common/landing/portal/admin/onboarding/privacy/donate (not flat-spread)", async () => {
    cookieGet.mockReturnValue({ value: "en" });
    const result = await callConfig();
    // Must have exactly these top-level namespace keys.
